...
export default async function middleware(req: NextRequest) {
  ...
  // Check for session cookie directly in edge runtime
  // Better Auth uses "better-auth.session_token" cookie by default
  // In production with secure cookies, it adds "__Secure-" prefix
  const sessionCookie =
  ...
  console.log('middleware debug:', {
    ...
    sessionCookie: sessionCookie?.value?.substring(0, 20) + '...',
    isLoggedIn,
  });
