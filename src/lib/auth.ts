 ...
 *
 * docs:
 * https://watermarkremover.tools/docs/auth
 * https://www.better-auth.com/docs/reference/options
 */
export const auth = betterAuth({
  baseURL:
    process.env.NODE_ENV === 'production'
      ? process.env.BETTER_AUTH_URL || 'https://mk-saas-main.vercel.app'
      : getBaseUrl(),
  ...
  trustedOrigins:
    process.env.NODE_ENV === 'production'
      ...
      : ['http://localhost:3000'],
  advanced: {
    cookiePrefix: 'better-auth', // Explicitly set the cookie prefix
    ...
    defaultRedirectURL:
      process.env.NODE_ENV === 'production'
        ? `${process.env.NEXT_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || 'https://mk-saas-main.vercel.app'}/dashboard`
        : '/dashboard',
    defaultCookieAttributes: {
      ...
      sameSite: 'lax',
      path: '/',
      // Do not set domain to allow cookies to work properly on Vercel
    },
  },
  session: {
  ...
  emailAndPassword: {
    ...
    requireEmailVerification: false, // Temporarily disable to test if this is the issue
  ...
  socialProviders: {
    ...
    google: {
      ...
      redirectURI:
        process.env.NODE_ENV === 'production'
          ? `${process.env.NEXT_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || 'https://mk-saas-main.vercel.app'}/api/auth/callback/google`
          : undefined,
    },
  },
  account: {
