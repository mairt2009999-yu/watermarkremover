...
export async function POST(req: Request) {
  try {
    ...
    const newPayment = {
      ...
      status: status,
      periodStart: now,
      periodEnd: planType === 'lifetime' ? null : new Date(now.getTime() + (interval === 'month' ? 30 : 365) * 24 * 60 * 60 * 1000),
...
export async function GET() {
  return NextResponse.json({
    ...
    description: 'This endpoint creates a payment record manually for debugging purposes'
  });
}