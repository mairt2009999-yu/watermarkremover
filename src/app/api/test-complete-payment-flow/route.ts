  ...
  try {
    ...
    const webhookPayload = {
      ...
      data: {
        ...
        product_id:
          process.env.NEXT_PUBLIC_CREEM_PRICE_PRO_MONTHLY || 'price_test',
        status: 'active',
        recurring: true,
        interval: 'month',
        metadata: {
    ...
    results.summary = {
      ...
      hasActiveSubscription: payments.some(
...
export async function GET() {
  return NextResponse.json({
    message:
      'POST to this endpoint with a user email to test the complete payment flow',
    example: {
      email: 'user@example.com',
    },
    description:
      'This will simulate a Creem webhook for the given user and check if payment records are created',
  });
}
